...
function passTokenToApiGetScoresApi() {
    console.log('passTokenToApi');
    getTokenPopup(tokenRequest)
        .then(response => {
            if (response) {
                console.log("access_token acquired at: " + new Date().toString());
                try {
                    getScoresApi(apiConfig.webApi, response.accessToken);
                  // apiFn(apiConfit.webApi, response.accessToken);
                } catch(error) {
                    console.log(error); 
                }
            }
...
function passTokenToSendScoreApi() {
    console.log('passTokenToApi');
    getTokenPopup(tokenRequest)
        .then(response => {
            if (response) {
                console.log("access_token acquired at: " + new Date().toString());
                try {
                    sendScoreApi(apiConfig.webApi, response.accessToken);
                  // apiFn(apiConfit.webApi, response.accessToken);
                } catch(error) {
                    console.log(error); 
                }
